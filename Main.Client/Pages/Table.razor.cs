    ...
    public partial class Table
    {
        ...
        void InsertRow(bool insertAbove)
        {
            if (!TableMeged())
            {
                ...
                var fontFamily = Bound.Page.ToolsBar.GetDefaultFont();
                for (var i = 1; i <= Data.HeaderCells.Count; i++)
                    row.Cells.Add(new TableCellData(row) { Font = new Font("12", fontFamily) });
        ...
        void RemoveRow()
        {
            ...
            else
            {
                if (Data.Rows.Count == 1)
                    ...
                    message = "For row removing we need to unmerge table";
            }
        }

        void InsertColumn(bool insertBefore)
        {
            if (!TableMeged())
            {
                Bound.Page.Stack.Push(Data);
                ...
                var tempIndex = insertBefore ? index - 1 : index;
                ...
                foreach (var row in Data.Rows)
                    row.Cells.Insert(tempIndex, new TableCellData(row) { Font = new Font("12", fontFamily) });
                UpdateRowAndColumnIndex();
            }
            else
                message = "For column inserting we need to unmerge table";
        }
        ...
        void SelectTableCell(MouseEventArgs e, TableCellData cell)
        {
            if (e.CtrlKey)
            {
            ...
            if (selectedCells.Count > 0)
                Bound.Page.SelectTable(this, selectedCells.Count == 1 && e.AltKey);
            else
                Bound.Page.ResetAll();
        }
        ...
        void UpdateTableLocation()
        {
            ...
            {
                Data.Left = Convert.ToInt32((BoundItem.ColumnWidth - TableWidth - 15) / 2);
                Data.Top = 0;
            }
        }
