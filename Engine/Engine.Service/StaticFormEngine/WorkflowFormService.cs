...
namespace Caspian.Engine.Service
{
    public class WorkflowFormService : BaseService<Engine.WorkflowForm>
    {
        public WorkflowFormService(IServiceProvider provider)
            :base(provider)
        {
            RuleFor(t => t.Title).Required().UniqueAsync("فرمی با این عنوان در سیستم ثبت شده است");
            ...
            RuleFor(t => t.Name).UniqueAsync("فرمی با این نام در سیستم ثبت شده است").Required().CustomValue(t => t.IsValidIdentifire(), "برای تعریف کلاس فقط از کاراکترهای لاتین و اعداد استفاده نمایید.");
        }
