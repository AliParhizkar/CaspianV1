...
namespace ReportGenerator.Services
{
    public static class ReportComponentExtension
    {
        ...
        public static async Task<XElement> GetXMLDocument(this ReportPageData pageData, IServiceProvider provider)
        {
            ...
            var hasSubReport = pageData.Bound.Items.Max(t => t.GetDataLevel()) == 3 && 
                pageData.Bound.Items.Select(t => t.ColumnsCount).Distinct().Count() > 1;
            var pages = new XElement("Pages").AddAttribute("isList", true).AddAttribute("count", hasSubReport ? 2 : 1);
            pages.Add(pageData.GetXMLElement(hasSubReport));
            if (hasSubReport)
                pages.Add(pageData.GetSubReportPageXMLElement());
            element.Add(pages);
        ...
        public static XElement GetXMLElement(this ReportPageData pageData, bool hasSubReport)
        {
            ...
            page.AddElement(pageData.Bound.GetXMLElement(pageData, hasSubReport));
        ...
        public static XElement GetSubReportPageXMLElement(this ReportPageData pageData)
        {
            var subreport = new XElement("SubReport_1").AddAttribute("Ref", 14).AddAttribute("type", "Page").AddAttribute("isKey", true);
            subreport.AddElement("Border", "None;[0:0:0];1;Solid;False;4;[0:0:0]");
            subreport.AddElement("Brush", "Transparent");
            var subreportBondsType = new BondType[] {BondType.SecondDataLevel, BondType.ThirdDataLevel, BondType.DataHeader, BondType.DataFooter };
            var items = pageData.Bound.Items.Where(t => subreportBondsType.Contains(t.BondType));
            var components = new XElement("Components").AddAttribute("isList", true).AddAttribute("count", items.Count());
            foreach(var item in items)
            {
                var xml = item.GetXMLElement(pageData, true, true);
                components.Add(xml);
            }
            subreport.Add(components);
            subreport.Add(new XElement("Conditions").AddAttribute("isList", true).AddAttribute("count", 0));
            subreport.Add(new XElement("Expressions").AddAttribute("isList", true).AddAttribute("count", 0));
            subreport.AddElement("Guid", "561a7f2a318c4a37b05daf7259466806");
            subreport.AddElement("Margins", "0,0,0,0");
            subreport.AddElement("Name", "SubReport_1");
            subreport.AddElement("PageHeight", pageData.Setting.PageHeight);
            var columnsCount = pageData.Bound.Items.Single(t => t.GetDataLevel() == 3).ColumnsCount;
            subreport.AddElement("PageWidth", pageData.Setting.PageWidth / columnsCount);
            subreport.Add(new XElement("Report").AddAttribute("isRef", 0));
            return subreport;
        }

        public static XElement GetXMLElement(this BoundData boundData, ReportPageData pageData, bool hasSubReport)
        {
            ...
            {
                var subreportBoundsType = new BondType[] {BondType.DataHeader, BondType.ThirdDataLevel, BondType.DataFooter };
                if (!hasSubReport || !subreportBoundsType.Contains(item.BondType))
                    bound.AddElement(item.GetXMLElement(pageData, hasSubReport));
            }
            return bound;
        }

        public static XElement GetSubreportXMLElement(BoundItemData boundItem, double pageWidth)
        {
            XElement element = new XElement("SubReport1").AddAttribute("Ref", Extension.CreateId()).AddAttribute("type", "SubReport")
                .AddAttribute("isKey", true);
            element.AddElement("Border", "None;[0:0:0];1;Solid;False;4;[0:0:0]");
            element.AddElement("Brush", "Transparent");
            element.AddElement("ClientRectangle", $"0,0,{pageWidth / boundItem.ColumnsCount},10".Replace('/', '0'));
            element.Add(new XElement("Components").AddAttribute("isList", true).AddAttribute("count", 0));
            element.Add(new XElement("Conditions").AddAttribute("isList", true).AddAttribute("count", 0));
            element.Add(new XElement("Expressions").AddAttribute("isList", true).AddAttribute("count", 0));
            element.AddElement("Name", "SubReport1");
            element.Add(new XElement("Parameters").AddAttribute("isList", true).AddAttribute("count", 0));
            element.Add(new XElement("Parent").AddAttribute("isRef", boundItem.BondType.ConvertToInt() + 1));
            element.Add(new XElement("Page").AddAttribute("isRef", 15));
            element.AddElement("SubReportPageGuid", "561a7f2a318c4a37b05daf7259466806");
            return element;
        }

        public static XElement GetXMLElement(this BoundItemData boundItemData, ReportPageData pageData, bool hasSubReport, bool isSubReport = false)
        {
            ...
            switch(boundItemData.BondType)
            {
                ...
                case BondType.ThirdDataLevel:
                    ...
                    if (isSubReport && boundItemData.BondType == BondType.SecondDataLevel)
                        id = 16;
            ...
            if (dataLevel.HasValue && (!isSubReport || boundItemData.BondType != BondType.SecondDataLevel))
            {
            ...
            if (boundItemData.ColumnsCount > 1 && !isSubReport)
                element.AddElement("Columns").AddContent(boundItemData.ColumnsCount);
            var controlsCount = (boundItemData.Table != null || hasSubReport && !isSubReport) ? 1 : boundItemData.Controls.Count;
            var controls = element.AddElement("Components").AddAttribute("isList", true).AddAttribute("count", controlsCount).Element("Components");
            if (hasSubReport && !isSubReport && boundItemData.GetDataLevel() == 2)
                controls.AddElement(GetSubreportXMLElement(boundItemData, pageData.Setting.PageWidth));
            else if (boundItemData.Table != null)
                controls.AddElement(boundItemData.Table.GetXMLElement(pageData));
            else
            {
                foreach (var control in boundItemData.Controls)
                    controls.AddElement(control.GetXMLElement(boundItemData, isSubReport));
            }
            ...
            var pageId = isSubReport ? 14 : 15;
            element.AddElement(new XElement("Page").AddAttribute("isRef", pageId));
            element.AddElement(new XElement("Parent").AddAttribute("isRef", pageId));
            return element;
        }
        ...
        public static XElement GetXMLElement(this ControlData control, BoundItemData boundItem, bool isSubReport)
        {
            ...
            var pageId = isSubReport ? 14 : 15;
            element.AddElement(new XElement("Page").AddAttribute("isRef", pageId));
            var parentId = boundItem.BondType.ConvertToInt() + 1;
            if (isSubReport && boundItem.BondType == BondType.SecondDataLevel)
                parentId = 17;
            element.AddElement(new XElement("Parent").AddAttribute("isRef", parentId));
            string text = null;
