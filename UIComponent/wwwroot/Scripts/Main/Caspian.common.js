        ...
        bindValue: function (dotnetHelper, control, controlType) {
            switch (controlType) {
                ...
                case 2:
                    ...
                    $(control).bind('onOpen', function () {

                    });
                    $(control).bind('onClose', function () {

                    });
                    $(control).bind('onFocus', function () {

                    });
                    $(control).bind('onBlure', function () {

                    });
                    break;
                case 3:
                    $(control).bind('changeValue', function () {
                        var dateValue = $(control).data('tDatePicker').value();
                        if (dateValue) {
                            var date = new $t.PersianDate_(dateValue).toMilady();
                            console.log(date)
                            dotnetHelper.invokeMethodAsync('SetDateValue', date);
                        } else
                            dotnetHelper.invokeMethodAsync('SetDateValue', null);
                    });
                    //dotnetHelper.invokeMethodAsync('SetValue', $(control).data('tDatePicker').value());
                    break;
                case 5:
                    $(control).find('>.t-header .fa-time').click(function () {
                        dotnetHelper.invokeMethodAsync('SetBoolValue', true);
                    });
                    break;
                case 6:
                ...
                case 7:
                    $(control).bind('change', function () {
                        dotnetHelper.invokeMethodAsync('SetBoolValue', $(control).data('tCheckBox').value());
                    });
                    break;
                case 8:
                    $(control).bind('changeValue', function (e, value) {
...
;
    ...
    $t.dropDown.prototype = {
        ...
        open: function (position) {
            ...
            $element.appendTo($(this.element).closest('.t-dropdown')[0]);
            var width;
            ...
            $element.width(width);
            var self_ = this;
            $element.find(selector).unbind('mouseenter').unbind('mouseleave').unbind('click')
                .bind('mouseenter', $t.hover).bind('mouseleave', $t.leave)
                .bind('click', function (e) {
                    if (self_.onClick)
                        self_.onClick($.extend(e, { item: $(e.target).closest('.t-item')[0] }));
                });
            var elementPosition = position.offset;
            elementPosition.left += 2
            elementPosition.top += position.outerHeight + 2;
            ...
            $('body').unbind('mousedown.closeList');
            $('body').bind('mousedown.closeList', function (e) {
                if (!$(e.target).closest('.t-animation-container').hasClass('t-animation-container'))
                    self_.close();
            });
        },
        ...
        scrollTo: function (item) {
            ...
            dropDown.childNodes[0].scrollTop = (dropDownScrollTop > itemOffsetTop)
                                    ? itemOffsetTop
