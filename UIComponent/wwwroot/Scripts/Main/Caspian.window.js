(function ($) {
    ...
    $t.fx.zoom.prototype = {
        play: function (options, end) {
            let $element = this.element.show();
            let parent = $element.offsetParent()[0];
            let left = ($(window).width() - $element.width()) / 2 - parent.offsetLeft;
            let index = 0;
    ...
    $t.window.prototype = {
            ...
            if (!this.overlayItem) {
                ...
                if (this.windowId == 0)
                    ...
                    $('.t-window').each(function () {
                        ...
                        if (win && win.windowId == self_.windowId - 2)
                            if (($(this).css('display')) == 'block')
                                $item.appendTo(this);     
                    });
                }
        ...
        open: function (e) {
            ...
            var count = 0;
            $('.t-window').each(t => {
                if (!$(this).find('.t-content').html())
                    $(this).remove();
            });
            $element.find('.t-window-content').height('auto');
            if (!$element.is(':visible'))
            ...
            if (this.modal) {
                ...
                if (this.effects.list.length > 0 && this.effects.list[0].name !== 'toggle')
                    overlay.css('opacity', 0.3).show().animate({ opacity: 0.5 }, 300);
                else
