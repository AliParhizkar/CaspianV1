(function ($) {
    ...
    $.extend($t, {
        calendar: function (element, options) {
            ...
            var header = new $t.stringBuilder()
                         .cat('<span class="t-link t-nav-prev ')
                         .catIf('t-state-disabled', this.currentView.compare(this.viewedMonth, minDate, false) <= 0)
                         .cat('">')
                        .cat('<span class="t-icon fa fa-arrow-left"></span></span><span class="t-link t-nav-fast">')
			             .cat(this.currentView.title(this.viewedMonth, this.pDate, this.PViewdMonth))
			             .cat('</span>')
			             .cat('<span class="t-link t-nav-next ')
                         .catIf('t-state-disabled', this.currentView.compare(this.viewedMonth, maxDate, true) >= 0)
                .cat('"><span class="t-icon fa fa-arrow-right"></span></span>');

            $('.t-header', this.element).html(header.string());
            var self_ = this;
            $(this.element).find('.fa-arrow-left').click(function () {
                self_.navigateToFuture();
            });
            $(this.element).find('.fa-arrow-right').click(function () {
                self_.navigateToPast();
            });
            /* header */
            ...
            $('.t-nav-fast:not(.t-state-disabled)').unbind('click.calenderfast');
            
            $('.t-nav-fast:not(.t-state-disabled)').bind('click.calenderfast', function () {
                self_.navigateUp();
            });
    ...
    $t.calendar.prototype = {
        bindEvent: function (viewIndex) {
            self_ = this;
            var $items = $(this.element).find('.t-content .t-link');
            $items.unbind('click.calenderBind');
            $items.bind('click.calenderBind', function (e) {
                self_.navigateDown(e, e.target, viewIndex);
            });
        },
        ...
        navigateVertically: function (viewIndex, viewedMonth, plunge, target, PViewdMonth) {
            if (viewedMonth instanceof Date);
            ...
            if (plunge) {
                ...
                var collapseCoordinates = {
                    ...
                    top: Math.floor(oldViewHeight * (index % 3) / 3) + 20
                };
                if (viewIndex == 2) {
                    collapseCoordinates = {
                ...
                $('.t-animation-container').height(235);
                newView.height(200);
                if (!this.stopAnimation) {
                    oldView.parent()
            		                   .animate($.extend({
        ...
        navigateDown: function (e, target, viewIndex) {
            ...
            if (currentViewIndex == 0) {
                ...
                if (!this.currentView)
                    this.currentView = $t.calendar.views[currentViewIndex];
    ...
    $.extend($t.calendar, {
        views: [{
            ...
            bodyP: function (viewdMonth, pDate, minDate, maxDate) {
                let html = (new $t.stringBuilder()).cat('<thead><tr class="t-week-header">');
                let firstDayIndex = $t.cultureInfo.firstDayOfWeek;
                let days = $t.cultureInfo.days;
                let abbrDays = $t.cultureInfo.abbrDays;
                let months = $t.cultureInfo.months;
                ...
                for (var weekRow = 0; weekRow < 6; weekRow++) {
                    ...
                    for (var day = 0; day < 7; day++) {
                        ...
                        if (date.compareDate(minDate) != -1 && date.compareDate(maxDate) != 1)
                            html.cat('<span class=\"t-link\">' + date.Day + '</span>').cat('</td>');
                    }
        ...
        html: function (viewedMonth, selectedDate, minDate, maxDate, urlFormat, dates) {
            ...
            return new $t.stringBuilder().cat('<div class="t-widget t-calendar">')
			                             ...
			                             .cat('<span class="t-link t-nav-prev">')
			                             .cat('<span class="t-icon t-arrow-prev"></span></span><span class="t-link t-nav-fast">')
			                             .cat($t.calendar.views[0].title(viewedMonth, this.pDate, this.PViewdMonth))
			                             .cat('</span>')
                                        .cat('<span class="t-link t-nav-next"><span class="t-icon fa fa-arrow-right"></span></span>')
			                             .cat('</div>')
