...
namespace Caspian.Report
{
    public partial class BoundItem: ComponentBase
    {
        ...
        public void GetRecDatas(IList<RecData> list, bool creatingMode)
        {
            ...
            var offsetLeft = creatingMode ? Bound.Left : 0;
            var result = ReportControls.Where(t => t != Bound.Page.SelectedControl).Select(t => new RecData()
            {
                Left = t.Data.Left + offsetLeft,
                Top = t.Data.Top + (creatingMode ? t.BoundItem.Top: 0),
                Width = t.Data.Width,
        ...
        public double MinHeight()
        {
            ...
            {
                if (control.Top + control.Height> max)
                    max = control.Top + control.Height;
            }
            return max;
        }
        ...
        public void UpdateHeight()
        {
            ...
            if (height > Data.Height)
            {
                var bondIndex = Bound.BoundItems.IndexOf(this);
                var index = 0;
                var difHeight = height - Data.Height;
                foreach (var item in Bound.BoundItems)
                {
                    if (index > bondIndex)
                    {
                        if (item.Table != null)
                            item.Table.Data.Top += difHeight;
                        foreach(var control in item.ReportControls)
                            control.Data.Top += difHeight;
                    }
                    index++;
                }
                Data.Height = height;
